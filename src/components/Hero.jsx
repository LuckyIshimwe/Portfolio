...
export default function Hero() {
    ...
    const tick = () => {
      ...
      if (!dR.current) {
        ...
        if (cR.current === word.length) {
          tR.current = setTimeout(() => { dR.current = true; tick(); }, 1800);
          return;
        }
      } else {
        cR.current--;
  ...
  return (
    <section id="hero" style={{
      ...
      padding: "80px 5% 60px", background: BG,
      ...
      <div className="hero-inner" style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", gap: "4rem" }}>

        {/* Text */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <h1 style={{ fontWeight: 800, fontSize: "clamp(2.8rem, 7vw, 6.5rem)", lineHeight: 1.02, margin: 0, letterSpacing: "-0.03em", color: WHITE }}>
            Lucky<br /><span style={{ color: ORANGE }}>Ishimwe</span>
          </h1>

          <div style={{ height: 56, display: "flex", alignItems: "center", marginTop: "1rem" }}>
            <span style={{ fontWeight: 600, fontSize: "clamp(1rem, 2.5vw, 1.8rem)", color: "rgba(255,255,255,0.75)" }}>
              {typed}
              <span style={{ borderRight: `3px solid ${ORANGE}`, marginLeft: 2, animation: "blink 1s step-end infinite" }}>&thinsp;</span>
            </span>
          </div>

          <p style={{ fontSize: "clamp(0.88rem, 2vw, 1rem)", color: "rgba(255,255,255,0.45)", lineHeight: 1.85, marginTop: "1.25rem", maxWidth: 500 }}>
          ...
          <div style={{ display: "flex", gap: "2rem", marginTop: "4rem", paddingTop: "3rem", borderTop: `1px solid ${BORDER}`, flexWrap: "wrap" }}>
            {STATS.map(({ num, label }) => (
              <div key={label}>
                <div style={{ fontWeight: 800, fontSize: "clamp(1.5rem, 4vw, 2rem)", color: ORANGE, lineHeight: 1, letterSpacing: "-0.02em" }}>{num}</div>
        ...
        {/* Photo */}
        <div className="hero-photo" style={{ flexShrink: 0, position: "relative", width: 320, height: 320 }}>
          <div style={{ position: "absolute", inset: -4, borderRadius: "50%", background: `conic-gradient(${ORANGE} 0deg, transparent 60deg, ${ORANGE} 120deg, transparent 180deg, ${ORANGE} 240deg, transparent 300deg, ${ORANGE} 360deg)`, animation: "spin 8s linear infinite", zIndex: 0 }} />
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .hero-inner div[style*="display: flex"][style*="gap: 12"] {
            justify-content: center;
          }
          .hero-inner div[style*="borderTop"] {
            justify-content: center;
          }
          .hero-photo {
